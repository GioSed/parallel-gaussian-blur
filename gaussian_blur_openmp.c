...
void gaussian_blur_omp_loops(int radius, img_t *imgin, img_t *imgout)
{
    ...
    // Process image
    #pragma omp parallel for private(j, row, col) shared(imgin, imgout, radius, width, height) num_threads(NUM_THREADS) schedule(dynamic)
    for (i = 0; i < height; i++) {
        for (j = 0; j < width; j++) {
...
void gaussian_blur_omp_tasks(int radius, img_t *imgin, img_t *imgout)
{
    ...
    // Process image using OpenMP tasks
...
double timeit(void (*func)(), int radius, img_t *imgin, img_t *imgout)
{
